...
```bash
serverless-crammer$ sam build --use-container
```
...
```bash
serverless-crammer$ sam logs -n StockCheckerFunction --stack-name "serverless-crammer" --tail
```
...
```bash
serverless-crammer$ pip install -r tests/requirements.txt --user
# unit test
serverless-crammer$ python -m pytest tests/unit -v
# integration test, requiring deploying the stack first.
# Create the env variable AWS_SAM_STACK_NAME with the name of the stack we are testing
serverless-crammer$ AWS_SAM_STACK_NAME="serverless-crammer" python -m pytest tests/integration -v
```
...
```bash
sam delete --stack-name "serverless-crammer"
```
